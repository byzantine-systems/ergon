...
WHERE
    state = 'available' AND UPPER(valid_period) = 'infinity';
...
CREATE TRIGGER jobs_versioning_trigger
    ...
    FOR EACH ROW
    EXECUTE FUNCTION temporal_versioning ();
...
CREATE PROPERTY GRAPH ergon.workflow VERTEX TABLES (
    ergon.jobs AS jobs KEY (id) LABEL job PROPERTIES (id, state, worker, queue)) EDGE TABLES (
