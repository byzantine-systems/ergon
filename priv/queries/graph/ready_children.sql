...
SELECT
    edges.child_id
FROM
    GRAPH_TABLE (ergon.workflow MATCH (parent IS job) -[IS triggers] -> (child IS job
        WHERE
            child.state = 'available') columns (child.id AS child_id, parent.state AS parent_state)) AS edges
GROUP BY
    edges.child_id
HAVING
    BOOL_AND(edges.parent_state = 'completed');
